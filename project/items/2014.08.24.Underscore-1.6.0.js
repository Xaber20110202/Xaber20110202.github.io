...
(function() {
  ...
  _.range = function(start, stop, step) {
    if (arguments.length <= 1)="" {="" stop="start" ||="" 0;="" start="0;" }="" step="arguments[2]" 1;="" var="" length="Math.max(Math.ceil((stop" -="" start)="" step),="" 0);="" idx="0;" range="new" array(length);="" while(idx="" <="" length)="" range[idx++]="start;" +="step;" return="" range;="" };="" function="" (ahem)="" functions="" ------------------="" reusable="" constructor="" for="" prototype="" setting.="" ctor="function(){};" create="" a="" bound="" to="" given="" object="" (assigning="" `this`,="" and="" arguments,="" optionally).="" delegates="" **ecmascript="" 5**'s="" native="" `function.bind`="" if="" available.="" _.bind="function(func," context)="" args,="" bound;="" @="" 1.29="" function.prototype.bind="" 返回一个指定了this="" 的="" 函数="" (nativebind="" &&="" func.bind="==" nativebind)="" nativebind.apply(func,="" slice.call(arguments,="" 1));="" (!_.isfunction(func))="" throw="" new="" typeerror;="" 1.30="" 获得参数func,="" context="" 以外的参数="" args="slice.call(arguments," 2);="" 1.31="" 还真有点叼呢="" 这个bind="" 在="" 不存在的情况下="" 1.32="" 返回的这个bound函数还接受两种情况="" 一个是函数调用="" 一个是构造函数new式调用="" 1.33="" 简单函数调用情况="" 指定this="" 为context="" (!(this="" instanceof="" bound))="" func.apply(context,="" args.concat(slice.call(arguments)));="" 1.34="" 构造函数形式调用情况="" 也就不管context什么事了="" 之所以在构造函数情况不直接return="" func="" 为避免_.bind(func)="" 返回的函数是一个引用而在一些情况下被修改="" 闭包中的ctor函数="" 指定="" prototype对象为func.prototype="" 即同一个="" 模拟="" 构造函数的场景="" ctor.prototype="func.prototype;" self="new" ctor;="" 1.35="" 在实例生成完之后="" self原型链上拥有了func.prototype将ctor.prototype上的属性重置="" 1.36="" 传递参数="" 再将构造函数func="" 中的this.name="" 这类="" 整合进="" self对象="" 如果构造函数有apply调用下有返回值(比如安全模式="" 内部="" 或者内部return="" 打断)的情况="" result="func.apply(self," 1.37="" 其实这里可以用_.isobject的="" 毕竟实现是一模一样="" (object(result)="==" result)="" result;="" 1.38="" 否则="" 返回self对象="" 且已经整合进实例属性的self对象="" self;="" partially="" apply="" by="" creating="" version="" that="" has="" had="" some="" of="" its="" arguments="" pre-filled,="" without="" changing="" dynamic="" `this`="" context.="" _="" acts="" as="" placeholder,="" allowing="" any="" combination="" be="" pre-filled.="" 8.01="" 用于整合出一个指定了相应参数(已传递)的="" 新函数="" 可以接受新的参数="" 用于放在原函数参数的后面="" 例：var="" add="function(a," b)="" b;="" add5="_.partial(add," 5);="" 指定了第一个参数位置传递的为5="" add5(10);="="> 15;
  ...
  _.throttle = function(func, wait, options) {
    ...
    return function() {
      ...
      if (remaining <= 0)="" {="" cleartimeout(timeout);="" timeout="null;" @="" 8.18="" 更新previous为当前时间戳="" previous="now;" 8.19="" 立即执行函数="" 然后设置result="" 返回值="" result="func.apply(context," args);="" context="args" =="" null;="" 8.20="" 而在wait=""> 0  、options.trailing !== false 
  ...
  _.template = function(text, data, settings) {
    ...
    // 然后又是没有匹配的  <li>  source 变成 " __p+='';_.each(people, function(name) { __p+=' </li><li>"
...
</%></%></%([\s\s]+?)%></%=([\s\s]+?)%></%-([\s\s]+?)%></%-1231231231231%></%([\s\s]+?)%></%=([\s\s]+?)%></%-([\s\s]+?)%></%-1231231231231%></%([\s\s]+?)%></%=([\s\s]+?)%></%-([\s\s]+?)%></%([\s\s]+?)%></%=([\s\s]+?)%></%-([\s\s]+?)%></%-([\s\s]+?)%></%=([\s\s]+?)%></%(.+?)%></%([\s\s]+?)%></|></':></=></=>